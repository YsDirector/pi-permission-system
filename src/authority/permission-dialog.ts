...
/**
 * Deadline for interactive permission prompts (select / input), in ms.
 * When the user does not respond within this window the dialog is dismissed
 * and the request is treated as DENIED (fail-closed dead-man's switch).
 *
 * Fork-local customization: upstream @gotgenes/pi-permission-system prompts
 * without a timeout; this fork adds one so a silent user never leaves a
 * dangerous request approved-by-default. See PATCHES.md.
 */
